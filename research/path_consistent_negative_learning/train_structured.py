...
def evaluate_test(
    ...
    *,
    ...
    test_indices = _candidate_indices_for_split(data, TEST_SPLIT)
    scores = _score_indices(
        model, data, test_indices, device=device, batch_size=batch_size
    )
    selected_queries = {}
    ...
    for query_index, query_key in enumerate(data.query_keys):
        ...
        all_shortest = (data.selected_labels[start:stop] | data.disputed_labels[start:stop]).tolist()
...
def train(
    *,
    ...
    labels, loss_mask = strategy_targets(data, strategy, seed=seed)
    ...
    train_indices = _candidate_indices_for_split(data, TRAIN_SPLIT, loss_mask)
    validation_indices = _candidate_indices_for_split(
        data, VALIDATION_SPLIT, loss_mask
    )
    ...
    metrics, query_rows = evaluate_test(
        model, data, device=device, batch_size=batch_size
    )
    ...
    result = {
        ...
        "active_training_candidate_count": int(
            loss_mask[_candidate_indices_for_split(data, TRAIN_SPLIT)].sum()
        ),
