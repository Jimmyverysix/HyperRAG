...
def generate(
    ...
    figure, axes = plt.subplots(
        1, 3, figsize=(7.2, 2.55), gridspec_kw={"width_ratios": [1.35, 0.85, 1.0], "wspace": 0.48}
    )
    ax = axes[0]
    ...
    for x_value, y_value, row in zip(rates, selected, rows):
        ax.annotate(row["dataset"], (x_value, y_value), xytext=(3, 2), textcoords="offset points", fontsize=6)
    ax.set_xlabel("采样冲突率（%）")
    ax.set_ylabel("冻结的 λ*")
