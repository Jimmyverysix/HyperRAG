...
\begin{table*}[t]
  ...
  \label{tab:main}
  \resizebox{0.88\textwidth}{!}{\input{generated/tables/main_proxy.tex}}
\end{table*}
...
相对逐题匹配的随机降权，路径一致加权的宏平均 answer reach@10 差值为 \TunedVsRandomReachDelta{} 个百分点（95\% CI [\TunedVsRandomReachCILow, \TunedVsRandomReachCIHigh]），selected-path PR-AUC 差值为 \TunedVsRandomPRDelta{} 个百分点（95\% CI [\TunedVsRandomPRCILow, \TunedVsRandomPRCIHigh]）。该对照表明覆盖收益不能仅由降低等量随机负例的权重解释，但路径定位同时引入了可测的原路径排序代价。表\ref{tab:main}同时报告 all-shortest-positive；强正标记的宏平均覆盖更高，排序代价也更大，因此结构可达性不足以替代语义相关性判断。

\subsection{RQ4：领域特异权重}

图\ref{fig:lambda}给出完整 selection 曲线、各领域冻结值及冲突率相关性。\MaskSelectedDomainCount{}/\DatasetCount{} 个领域选择了屏蔽端点 $\lambda=0$，\InteriorSelectedDomainCount{} 个领域选择中间权重，\BaselineSelectedDomainCount{} 个领域选择基线端点。采样冲突率与 $\lambda_D^*$ 的 Spearman $\rho=\LambdaConflictRho$（$p=\LambdaConflictP$）。这是仅含 \DatasetCount{} 个领域的探索性分析，不作因果解释，也不据此修改冻结网格或删除不显著结果。
...
单路径弱监督把一个可观测选择过程误当成了完整的负例标注。完整主题---答案距离判据能够定位真实进入损失的路径一致负例；数据集级 $\lambda$ 重加权则在不改模型、标签和推理的条件下校准其监督强度。冻结测试中，该方案相对基线将 answer reach@10 宏平均提高 \TunedReachDelta{} 个百分点，selected-path PR-AUC 下降 \TunedPRDelta{} 个百分点；相对等量随机降权的覆盖收益区间下界仍为正。因此，本文在结构代理检索器上验证了路径定位机制和宏平均覆盖---排序折中。但这不是普遍无代价的替换：排序差值的 95\% 区间下界为 \TunedPRCILow{} 个百分点，且 \PRGuardrailViolationCount{}/\DatasetCount{} 个领域的点估计低于 $-1$ 个百分点。结论限于当前冻结协议和结构代理，不外推为官方 HyperRAG 端到端 QA 已得到验证。
...
\begin{table}[h]
  \centering
  \caption{逐领域路径冲突审计。}
  \resizebox{\columnwidth}{!}{\input{generated/tables/audit.tex}}
\end{table}

\begin{table*}[b]
  \centering
  \caption{selection split 上的 $\lambda$ 灵敏度（answer reach@10，百分数）。}
  \resizebox{0.78\textwidth}{!}{\input{generated/tables/lambda.tex}}
\end{table*}
