...
我们使用官方 WikiTopics\_QE 的 11 个领域。对每个领域读取训练图、三跳查询、困难答案和原始映射；按照公开的知识图谱到文本规则，将同一头实体的外向事实聚合为多元超边。作者没有发布最终 GraphML，故这里是基于官方整数图的确定性结构重建，而非原运行产物的字节级复原。对每个问题重建单条最短路径正例与引导子图，再从同一候选池复现均衡负采样。等长路径按固定字典序择一；原代码则会受 GraphML 节点插入顺序影响。审计使用共享随机种子 42--46，同时报告采样前候选池发生率和实际采样负例发生率。
...
公开仓库未提供论文训练所使用的最终 GraphML 与预训练文本表示，服务器上也没有相应模型缓存。因此，结构审计与门槛 C 都不冒充原版端到端 HyperRAG 复现，而是使用官方整数图构造结构化代理实验。模型输入只包含实体标识、查询关系、超边关系集合及不依赖标签的结构数值特征；共享嵌入经多层感知机输出候选分数。四个实验臂只改变标签或损失掩码。
...
门槛 B 预先设为：全局实际采样争议率达到 1\%，或任一领域达到 1\%，才进入训练。门槛 C 的主比较为策略2相对策略1和随机丢弃的答案可达率@10。先对每个问题的五个共享种子等权平均，再以唯一问题为配对单位执行 10,000 次配对自助法；两个比较的 95\% 区间下界都必须大于零。同时，策略2相对策略1的选中路径精确率--召回率曲线下面积下降不得超过 1 个百分点。只有三个条件同时成立，才扩展到门槛 D。逐种子的配对差值另外保留在机器可读汇总中。
...
策略2相对策略1的答案可达率@10配对差为 \VsStrategyOneDifference 个百分点，95\% 配对自助法区间为 [\VsStrategyOneCILow, \VsStrategyOneCIHigh]，共 \VsStrategyOnePairs 个问题配对。相对随机丢弃的差为 \VsRandomDropDifference 个百分点，区间为 [\VsRandomDropCILow, \VsRandomDropCIHigh]，共 \VsRandomDropPairs 个问题配对。策略2相对策略1的选中路径曲线下面积差为 \StrategyTwoPrecisionDifference 个百分点。

表\ref{tab:gate-c}呈现出清楚的权衡：随机丢弃只比策略1略高，而策略2的答案可达率明显高于二者，说明变化不能仅由负例数量减少解释；策略3的答案可达率最高，表明更积极的最短路径监督进一步扩大了结构覆盖。另一方面，四个实验臂的全部最短路径召回率@10几乎相同，而策略2和策略3的选中路径曲线下面积依次降低。也就是说，更宽的结构覆盖没有转化为对原选中证据排序的同步改善。
...
本文有四项边界。第一，审计和门槛 C 都是官方结构数据上的确定性重建，不包含未发布的 GraphML、文本编码器和生成器，因而不能报告端到端答案精确匹配率。第二，原 GraphML 的节点插入顺序未知，等长路径择一可能改变哪些候选被称为“未选中”；本文固定字典序以保证可复现，但没有声称复原原运行的择一路径。第三，路径一致是相对于观测图的结构性质；缺失边和伪边会分别拉长或缩短距离。第四，提案中的双人盲法语义标注需要独立人工标注者，本轮计算实验未获得该资源，因而没有把单个模型的判断伪装成人类一致性结果。本文对“语义有用证据占比”不作数值结论。
...
代码采用模块化目录，结构审计、数据准备、四臂训练、六卡调度、配对统计和绘图彼此分离。所有代码通过 GitHub 研究分支同步；服务器运行产物保留在独立实验目录。实验环境为 Python 3.11.15、PyTorch 2.2.2 与 CUDA 12.1，训练并发使用 6 张 24,GB 的 RTX 3090。门槛 A 由 41 项单元测试覆盖，门槛 B 与门槛 C 的判定均由聚合脚本从原始运行结果自动生成，论文表格中的数字再由同一汇总文件生成，避免手工转录。
